...
- 判断: GitHub App Keychainはloginキーチェーン確認後にデフォルト検索リストへフォールバックする。
- 理由: `B_Stats_Site`の`work_record_023`（2026-08-24）と`work_record_028`（2026-08-27）では同じservice名・ユーザー名・PEM方式で取得成功していた一方、現在は同リポジトリの`--check-keychain`も失敗したため、保存先がlogin以外または検索リスト側にある可能性を残す必要がある。
